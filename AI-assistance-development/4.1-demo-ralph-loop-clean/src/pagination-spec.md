...
`paginate()` must handle a 100,000-item dataset and return a single page in under
60 seconds. This is a hard ceiling verified by a benchmark test
(`tests/pagination.perf.test.ts`), not a runtime-enforced timeout — there is no
timer or thrown error inside `paginate()` itself. In practice, a correct
implementation should resolve in milliseconds; 60s only exists to catch
accidental algorithmic regressions (e.g. re-scanning the full array from the
start on every call, or other patterns that scale worse than necessary).
...
All 7 functional test cases plus the performance benchmark must pass.
